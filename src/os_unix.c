...
FileType getFileType(const char* path) {
  struct stat info;
  if (stat(path, &info) == -1) return FT_INVALID;
  if (S_ISCHR(info.st_mode)) return FT_DEV;
  if (S_ISDIR(info.st_mode)) return FT_DIR;
  if (S_ISREG(info.st_mode)) return FT_REG;
  return FT_INVALID;
}
...
const char* dirGetName(const DirIter* iter) {
  if (iter->error || !iter->entry) return NULL;
...
char* getFullPath(const char* path) {
  ...
  if (realpath(path, resolved_path) == NULL) {
    ...
    if (realpath(parent_dir, resolved_parent_dir) == NULL) return NULL;
