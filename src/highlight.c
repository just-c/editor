...
void editorUpdateSyntax(EditorFile* file, EditorRow* row) {
  ...
  if (!CONVAR_GETINT(syntax) || !s) goto update_trailing;
  ...
  while (i < row->size) {
    ...
    if (s->flags & HL_HIGHLIGHT_NUMBERS) {
      if ((isdigit(c) || c == '.') && prev_sep) {
        ...
        if (c == '.' && i - start == 1) continue;

        if (row->data[i] == 'f' || row->data[i] == 'F') i++;
    ...
    if (prev_sep) {
      ...
      for (int kw = 0; kw < 3; kw++) {
        for (size_t j = 0; j < s->keywords[kw].size; j++) {
          ...
          if (strncmp(&row->data[i], s->keywords[kw].data[j], klen) == 0 &&
...
void editorInitHLDB(void) {
  ...
  if (iter.error) return;

  do {
    const char* filename = dirGetName(&iter);
    char file_path[EDITOR_PATH_MAX];
    int len = snprintf(file_path, sizeof(file_path), PATH_CAT("%s", "%s"), path,
                       filename);
...
void editorFreeHLDB(void) {
  ...
  while (HLDB) {
    ...
    for (size_t i = 0; i < sizeof(temp->keywords) / sizeof(temp->keywords[0]);
         i++) {
