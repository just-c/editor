...
static void json__arena_alloc_new_block(JsonArena* arena,
  ...
  if (allocated_size > UINT32_MAX) allocated_size = UINT32_MAX;
...
void* json_arena_alloc(JsonArena* arena, size_t size) {
  ...
  if (!arena->blocks) json__arena_alloc_new_block(arena, size);
...
void* json_arena_realloc(JsonArena* arena, void* ptr, size_t old_size,
  ...
  if (!ptr || !arena->blocks) return json_arena_alloc(arena, size);
    ...
    if (c == ' ' || c == '\t' || c == '\n' || c == '\r') {
      ...
      if (c == '-') {
        ...
        if (!(c >= '0' && c <= '9')) {
          return json__token_error(state,
                                   "No number after minus sign at position %ld",
                                   state->p - state->start);
        }
      }
      ...
      if (c == 'e' || c == 'E') {
        ...
        if (c >= '0' && c <= '9') {
          ...
          return json__token_error(
              state, "Exponent part is missing a number at position %ld",
              state->p - state->start);
        }
      }
      state->p--;
      ...
      while ((c = *state->p++) != '"') {
        ...
        if (c == '\\') {
          ...
          switch (c) {
            ...
            default:
              return json__token_error(state,
                                       "Bad escaped character at position %ld",
                                       state->p - state->start);
          }
        }

        if (size + 2 > capacity) {
          token.string = json_arena_realloc(state->arena, token.string,
                                            capacity, capacity * 2);
          capacity *= 2;
        }
      ...
      switch (c) {
        ...
        default:
          return json__token_error(state,
                                   "Unexpected token '%c' at position %ld", c,
                                   state->p - state->start);
      }
      return token;
    }
...
static JsonValue* json__parse_array(JsonParserState* state) {
  ...
  while (token.type != JSON_TOKEN_RBRACKET) {
    ...
    if (data->type == JSON_ERROR) return data;
...
JsonValue* json_parse(const char* text, JsonArena* arena) {
  ...
  if (value->type == JSON_ERROR) return value;
